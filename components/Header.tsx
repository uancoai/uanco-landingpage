    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-gray-100/50">
      <div className="max-w-7xl mx-auto px-6 h-16 md:h-20 flex items-center justify-between">
        ...
        <div className="flex-shrink-0">
          ...
          >
            <span className="text-2xl md:text-[28px] font-serif italic font-bold text-[#1A1A1A] lowercase tracking-tighter leading-none">
              uanco
            </span>
            <span className="w-1.5 h-1.5 bg-[#1A1A1A] rounded-full mb-[3px] md:mb-[4px] ml-0.5"></span>
          </button>
        </div>
        ...
        <div className="hidden md:flex items-center gap-8 lg:gap-10 absolute left-1/2 -translate-x-1/2">
          {NAVIGATION.map((item) => (
            <button 
              ...
              className="text-[13px] font-medium text-gray-400 hover:text-[#1A1A1A] transition-colors outline-none"
            >
              {item.label}
            </button>
          ))}
        ...
        {/* Right spacing for desktop layout balance */}
        <div className="flex-shrink-0 w-[60px] hidden md:block"></div>
      </div>
    </nav>
  );
};
