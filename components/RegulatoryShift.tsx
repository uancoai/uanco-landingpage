...
const RegulatoryShift: React.FC = () => {
  ...
  return (
    <section className="pt-8 pb-0 md:py-24 lg:py-40 bg-white overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto md:px-6 relative z-10">
        {/* Container: Full width on mobile (rounded-none), Pill on desktop (md:rounded-[4rem]) */}
        <div className="bg-[#1A1A1A] rounded-none md:rounded-[4rem] p-8 md:p-16 lg:p-24 relative overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)]">
          {/* Architectural Background Pattern */}
          <div className="absolute top-0 right-0 w-1/2 h-full bg-white/5 -skew-x-12 translate-x-1/2 hidden md:block"></div>
          
          <div className="relative z-10 grid lg:grid-cols-2 gap-10 md:gap-20 items-start">
            <div>
              ...
              <h2 className="text-3xl md:text-5xl lg:text-7xl font-bold text-white mb-6 md:mb-8 tracking-tight leading-tight">
                Why this <br />
              ...
              {items.map((item, i) => (
                ...
                >
                  <button 
                    ...
                    className="w-full flex items-center justify-between p-5 md:p-8 text-left outline-none group"
                    aria-expanded={openIndex === i}
                  >
                    <h4 className="text-white font-bold text-sm md:text-lg group-hover:text-zinc-300 transition-colors">
                      {item.title}
                    </h4>
                    <ChevronDown 
                      size={18} 
                      className={`text-zinc-500 transition-transform duration-500 ease-in-out ${openIndex === i ? 'rotate-180 text-white' : ''}`} 
                    />
                  </button>
